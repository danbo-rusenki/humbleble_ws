...
#include "behavior_tree_msgs/action/after_arm.hpp" // 着陸アクションのためのカスタムアクション型
...
class AfterArm : public rclcpp::Node
{
public:
    // エイリアス定義でコードの可読性を向上
    using AfterArmAction = behavior_tree_msgs::action::AfterArm;
    using GoalHandleLand = rclcpp_action::ServerGoalHandle<AfterArmAction>;

    // コンストラクタ: ノードの初期化とパブリッシャー、サブスクライバー、アクションサーバーの設定
    AfterArm()
        : Node("AfterArm_action_server") // ノード名を指定
    {
        ...
        after_arm_sub_ = this->create_subscription<amir_interfaces::msg::AmirSensor>("/encoder_pub", qos_profile, std::bind(&AfterArm::angle_callback, this, std::placeholders::_1));

        // アクションサーバーを初期化
        action_server_ = rclcpp_action::create_server<AfterArmAction>(
            this, // 現在のノードを指定
            "amir/bt_after_arm", // アクション名
            std::bind(&AfterArm::handle_goal, this, std::placeholders::_1, std::placeholders::_2), // ゴール受付コールバック
            std::bind(&AfterArm::handle_cancel, this, std::placeholders::_1), // キャンセル受付コールバック
            std::bind(&AfterArm::handle_accepted, this, std::placeholders::_1)); // ゴール処理コールバック

        // アクションサーバーの起動ログを出力
        RCLCPP_INFO(this->get_logger(), "****** AfterArm action server started *****");
    }
...
private:
    ...
    // アクションサーバー
    rclcpp_action::Server<AfterArmAction>::SharedPtr action_server_;
    // サブスクライバー: 目標手先位置、オブジェクトの座標？を取得
    rclcpp::Subscription<amir_interfaces::msg::AmirSensor>::SharedPtr after_arm_sub_;
    ...
    rclcpp_action::GoalResponse handle_goal(
        ...
        std::shared_ptr<const AfterArmAction::Goal> goal) // ゴールの内容
    {
        (void)uuid;
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Executing AfterArm action");

        auto result = std::make_shared<AfterArmAction::Result>(); // 実行結果を格納する変数
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<AfterArm>(); // MoveArmノードを作成
