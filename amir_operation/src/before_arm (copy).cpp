...
#include "behavior_tree_msgs/action/before_arm.hpp" // 着陸アクションのためのカスタムアクション型
...
public:
    ...
    BeforeArm()
        : Node("BeforeArm_action_server") // ノード名を指定
    {
        ...
        before_arm_sub_ = this->create_subscription<amir_interfaces::msg::AmirSensor>("/encoder_pub", qos_profile, std::bind(&BeforeArm::angle_callback, this, std::placeholders::_1));

        // アクションサーバーを初期化
        action_server_ = rclcpp_action::create_server<BeforeArmAction>(
            this, // 現在のノードを指定
            "amir/bt_before_arm", // アクション名
            std::bind(&BeforeArm::handle_goal, this, std::placeholders::_1, std::placeholders::_2), // ゴール受付コールバック
            std::bind(&BeforeArm::handle_cancel, this, std::placeholders::_1), // キャンセル受付コールバック
            std::bind(&BeforeArm::handle_accepted, this, std::placeholders::_1)); // ゴール処理コールバック
...
private:
    ...
    rclcpp::Subscription<amir_interfaces::msg::AmirSensor>::SharedPtr before_arm_sub_;
    ...
    rclcpp_action::GoalResponse handle_goal(
        ...
        std::shared_ptr<const BeforeArmAction::Goal> goal) // ゴールの内容
    {
        (void)uuid;
    ...
    void execute(const std::shared_ptr<GoalHandleLand> goal_handle, float P_x, float P_y, float P_z)
    {
        ...
        auto result = std::make_shared<BeforeArmAction::Result>(); // 実行結果を格納する変数
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<BeforeArm>(); // MoveArmノードを作成
