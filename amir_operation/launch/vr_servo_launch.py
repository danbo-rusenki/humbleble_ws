...
前提: 実機 or sim の bringup が起動済みで以下が稼働中
  - robot_state_publisher
  - controller_manager (実機=実機 ros2_control_node / sim=gz_ros2_control)
  - joint_state_broadcaster
  - arm_controller (JointTrajectoryController)  ← Servo 運用時は deactivate する
  - gripper_controller

use_sim_time は既定 false（実機）。sim で使うときは use_sim_time:=true を付ける。
  例) ros2 launch amir_operation vr_servo_launch.py use_sim_time:=true
