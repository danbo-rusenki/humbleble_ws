...
#ifndef ROS2_BEHAVIOR_TREE__CONTROL__PRIORITY_FALLBACK_NODE_HPP_
#define ROS2_BEHAVIOR_TREE__CONTROL__PRIORITY_FALLBACK_NODE_HPP_
...
class PriorityFallbackNode : public BT::ControlNode
{
public:
  explicit PriorityFallbackNode(const std::string & name)
  : BT::ControlNode(name, {})
  {
  }

  PriorityFallbackNode(const std::string & name, const BT::NodeConfiguration & config)
...
protected:
  ...
  BT::NodeStatus tick() override
  {
    ...
    if (!getInput<std_msgs::msg::Float64MultiArray>("priority", priority_array)) {
      throw BT::RuntimeError("Missing parameter [priority] in PriorityFallbackNode");
    }
    //  優先度の配列サイズと子ノードのサイズが異なる場合エラー
    if (priority_array.data.size() != children_nodes_.size()){
      throw BT::RuntimeError("Invalid parameter [priority] in PriorityFallbackNode");
    }
...
#endif  // ROS2_BEHAVIOR_TREE__CONTROL__PRIORITY_FALLBACK_NODE_HPP_
