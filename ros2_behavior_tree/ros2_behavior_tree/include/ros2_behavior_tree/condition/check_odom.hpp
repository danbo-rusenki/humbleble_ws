...
#ifndef ROS2_BEHAVIOR_TREE__CONDITION__CHECK_ODOM_NODE_HPP_
#define ROS2_BEHAVIOR_TREE__CONDITION__CHECK_ODOM_NODE_HPP_
...
{
public:
  CheckOdomNode(const std::string & name, const BT::NodeConfiguration & config)
  ...
  static BT::PortsList providedPorts()
  {
    return {
      ...
      BT::InputPort<std::string>("odom_check",std::string("/odom") ,""),
  ...
  BT::NodeStatus tick() override
  {
    ...
    while (!rclcpp::wait_for_message(odom_topic_,node_,"/odom",server_timeout_))
    {
      RCLCPP_DEBUG(node_->get_logger(),"timeout /odom topic");
