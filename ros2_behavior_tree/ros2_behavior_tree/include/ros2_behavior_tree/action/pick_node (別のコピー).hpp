...
#include "ros2_behavior_tree/bt_action_node.hpp"
...
class PickNode : public ros2_behavior_tree::BtActionNode<Pick>
{
public:
  PickNode(const std::string & name, const BT::NodeConfiguration & config)
  : BtActionNode<Pick>(name, name, config)
  {
  }
  ...
  void on_tick()
  {
    if(!getInput<std::string>("obj_id", goal_.id))
    {
      throw BT::RuntimeError("Missing parameter [obj_id] in PickNode node");
    }
    
    if (!getInput<geometry_msgs::msg::PoseStamped>("obj_pose", goal_.pose)) {
  ...
  BT::NodeStatus on_success()
  {
    ...
    grasped_object = result_.result->grasped_object;
    
    if (result_.result->error_string != "success") grasped_object.object.id = "empty";
 
    /*発見した物体位置を出力*/
    setOutput<moveit_msgs::msg::AttachedCollisionObject>("grasped_object", grasped_object);
    setOutput<std::string>("pick_error_string", result_.result->error_string);
    ...
    {
      RCLCPP_INFO(node_->get_logger(),"error_string = %s", result_.result->error_string.c_str());
