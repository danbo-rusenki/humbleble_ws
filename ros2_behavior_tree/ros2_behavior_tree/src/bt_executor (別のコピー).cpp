    ...
    void execute(const std::shared_ptr<GoalHandleExecuteTree> goal_handle)
    {
      ...
      while(bt_root_status_ == BT::NodeStatus::IDLE || bt_root_status_ == BT::NodeStatus::RUNNING && rclcpp::ok())
      {
        ...
        if (goal_handle->is_canceling()) {
          bt_->haltTree(); //  BT停止
          result->error_string = "cancel";
          goal_handle->canceled(result);
          RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Goal cancelled");
          return;
        }

        if(tree_updated_ == true)
        {
          goal_handle->succeed(result);
          RCLCPP_INFO(node_->get_logger(), "Goal Update");
        }
